              ...
              <Form ref="menuForm" :model="menuForm" :label-width="85" :rules="menuFormValidate">
                ...
                <div v-if="menuForm.type===0">
                  ...
                  <FormItem label="前端组件" prop="component">
                    <Input :value="`/pages${menuForm.path}`" disabled/>
                  </FormItem>
...
export default {
  ...
  methods: {
    ...
    search() {
      if (this.searchKey) {
        this.loading = true;
        this.$axios.api.searchPermission({ title: this.searchKey }).then(res => {
          this.loading = false;
          if (res.message === 'success') {
            this.data = res.data;
          }
    ...
    submitEdit() {
      this.$refs.menuForm.validate(valid => {
        if (valid) {
          ...
          this.$axios.api.editPermission(this.menuForm).then(res => {
    ...
    submitAdd() {
      this.$refs.menuFormAdd.validate(valid => {
        if (valid) {
          ...
          this.$axios.api.addPermission(this.menuFormAdd).then(res => {
            this.submitLoading = false;
            if (res.message === 'success') {
