...
export default {
  ...
  methods: {
    ...
    getRoleList() {
      ...
      this.$axios.api.getRoleList(params).then(res => {
        this.loading = false;
    ...
    getPermList() {
      ...
      this.$axios.api.getAllPermissionList().then(res => {
      ...
      this.$refs.roleForm.validate(valid => {
        if (valid) {
          if (this.modalType === 0) {
            ...
            this.$axios.api.addRole(this.roleForm).then(res => {
            ...
            this.submitLoading = true;
            this.$axios.api.editRole(this.roleForm).then(res => {
              this.submitLoading = false;
              if (res.message === 'success') {
                this.$Message.success("操作成功");
                this.getRoleList();
    ...
    setDefault(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          this.operationLoading = true;
          this.$axios.api.setDefaultRole(params).then(res => {
            this.operationLoading = false;
            if (res.message === 'success') {
              this.$Message.success("操作成功");
              this.getRoleList();
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          this.operationLoading = true;
          this.$axios.api.deleteRole(ids).then(res => {
            this.operationLoading = false;
            if (res.message === 'success') {
              this.$Message.success("删除成功");
              this.clearSelectAll();
              this.$axios.api.getRoleList();
            }
          });
        }
      });
    ...
    getParentList() {
      ...
      this.$axios.api.initDepartment({ openDataFilter: false }).then(res => {
    ...
    loadData(item, callback) {
      this.$axios.api.loadDepartment(item.id, { openDataFilter: false }).then(res => {
        if (res.message === 'success') {
          res.data.forEach(function(e) {
            e.selected = false;
            if (e.isParent) {
